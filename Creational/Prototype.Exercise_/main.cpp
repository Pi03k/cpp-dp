...
class GraphicsDoc
{
  ...
  GraphicsDoc(const GraphicsDoc& copy)
    : shape_factory_(copy.shape_factory_), shape_rw_factory_(copy.shape_rw_factory_)
  {
    shapes_.clear();
    for(auto& shape : copy.shapes_)
    {
      shapes_.push_back(shape->clone());
    }
  }

  GraphicsDoc& operator=(const GraphicsDoc& copy)
  {
    if (&copy == this)
      return *this;
    this->shapes_.clear();
    ...
    this->shape_factory_ = copy.shape_factory_;
    this->shape_rw_factory_ = copy.shape_rw_factory_;
  }
